...
static int display_pre_init(void)
{
	...
	list_for_each_entry_reverse(state, &rockchip_display_list, head) {
		struct connector_state *conn_state = &state->conn_state;
...
static int rockchip_read_bmp_file(void *bmp_file,const char *bmp_name,int size)
{
	...
	sprintf(logo_hex_str, "0x%lx", (ulong)bmp_file);
	sprintf(r_size, "%x", size);
	...
	//fs_argv[4] = bmp_name;
	strcpy(fs_argv[4],bmp_name);
	fs_argv[5] = r_size;
	
	debug("do load file=%s %s:%s:%s size:%s\n",bmp_name,devtype,devnum_part,logo_hex_str,r_size);
...
static int load_bmp_logo(struct logo_info *logo, const char *bmp_name)
{
	...
	//header = malloc(RK_BLK_SIZE);
	debug("%s..%d:%d\n",__func__,ARCH_DMA_MINALIGN,RK_BLK_SIZE);
//	header = memalign(ARCH_DMA_MINALIGN,RK_BLK_SIZE/ARCH_DMA_MINALIGN);
	header = malloc_cache_aligned(RK_BLK_SIZE);
	if (!header)
		return -ENOMEM;
	...
	if (len != RK_BLK_SIZE) {
		ret = -EINVAL;
		//goto free_header;
	//	bmp_file = memalign(ARCH_DMA_MINALIGN,1024*50);
	//	if (!bmp_file){
	//		printf("malloc fail \n");
	//		ret = -ENOMEM;
	//		goto free_header;
	//	}
	//	ret = rockchip_read_bmp_file(bmp_file,bmp_name);
		ret = rockchip_read_bmp_file(header,bmp_name,RK_BLK_SIZE);
		if(ret)
			goto free_header;
	...
	dst_size = logo->width * logo->height * logo->bpp >> 3; //需要多少个字节显示LOGO
	reserved = get_unaligned_le32(&header->reserved);
	
	if (logo->height < 0)
	    logo->height = -logo->height;
	
	size = get_unaligned_le32(&header->file_size);//BMP文件 大小
	
	debug("logo file para bpp:w:h:file size= %d:%d:%d:%d\n",logo->bpp,logo->width,logo->height,size);
	if (!can_direct_logo(logo->bpp)) { //bpp 不等于 24或32
		if (size > MEMORY_POOL_SIZE) {
			printf("failed to use boot buf as temp bmp buffer\n");
			ret = -ENOMEM;
			goto free_header;
		}
		pdst = get_display_buffer(size); //获取BMP文件 大小的buffer
		
		printf("display buffer 0x%p \n",pdst);
	...
	if (len != size) {
		...
		ret = -ENOENT;
	//	goto free_header;
	//	pdst = bmp_file; //直接指向文件 位置
		rockchip_read_bmp_file(pdst,bmp_name,size);
		ret = 0;
	}
	...
	if (!can_direct_logo(logo->bpp)) {
		 ...
		 */
		logo->bpp = (logo->bpp <= 16) ? 16 : logo->bpp; //强制16位bpp格式 
		dst_size = logo->width * logo->height * logo->bpp >> 3; //图像所需要区域
		dst = get_display_buffer(dst_size); //分配buffer
		if (!dst) {
			ret = -ENOMEM;
			goto free_header;
		}
		if (bmpdecoder(pdst, dst, logo->bpp)) { //将图像copy到display区域
...
int rockchip_show_bmp(const char *bmp)
{
	...
	list_for_each_entry_reverse(s, &rockchip_display_list, head) {
		...
		printf("display_logo\n");
		ret = display_logo(s);
		printf("charge current type: %d\n",conn_state->type);
    		printf("charge current hpd: %d\n", conn_state->hdmi_detect);
    		if(conn_state->type == DRM_MODE_CONNECTOR_HDMIA && conn_state->hdmi_detect )
        		return ret;
	}
